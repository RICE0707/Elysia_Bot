...
module.exports = {
  ...
  command: {
    ...
    subcommands: [
      ...
      {
        trigger: "頻道 <頻道>",
        description: "設置發送成員入群群訊息的頻道",
      },
      {
        trigger: "預覽",
        description: "預覽已設置的成員入群群訊息",
      },
      {
        trigger: "描述 <描述>",
      ...
      {
        trigger: "顏色 <二進制色碼>",
        description: "設置成員入群群訊息的顏色",
      },
      {
        trigger: "頁腳 <文字>",
        description: "設置成員入群群訊息頁腳的文字",
      },
      {
        trigger: "圖片 <圖片網址>",
        description: "設置成員入群群訊息的圖片",
      },
    ],
  },
  slashCommand: {
...
async function sendPreview(settings, member) {
  if (!settings.welcome?.enabled) return "> <a:r2_rice:868583626227478591> 這個伺服器未啟用加入訊息。";

  const targetChannel = member.guild.channels.cache.get(settings.welcome.channel);
  if (!targetChannel) return "> <a:r2_rice:868583626227478591> 未設置發送加入訊息的頻道。";

  const response = await buildGreeting(member, "WELCOME", settings.welcome);
  await targetChannel.safeSend(response);

  return `> <a:r3_rice:868583679465758820> 已發送加入訊息預覽至 ${targetChannel.toString()} 頻道。`;
}
