  ...
  client.counterUpdateQueue.forEach(async (guildId) => {
    ...
    try {
      ...
      for (const config of settings.counters) {
        ...
        let channelName;
        if (config.counter_type.toUpperCase() === "總人數") channelName = `${config.name} : ${all}`;
        if (config.counter_type.toUpperCase() === "成員數") channelName = `${config.name} : ${members}`;
        if (config.counter_type.toUpperCase() === "機器數") channelName = `${config.name} : ${bots}`;
...
async function init(guild, settings) {
  if (settings.counters.find((doc) => ["成員數", "機器數"].includes(doc.counter_type.toUpperCase()))) {
